...
void AEnemy::HitPlayer(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult &SweepResult)
{
	GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, FString::Printf(TEXT("Player Hit")));
	AstarbreezeCharacter *Player = Cast<AstarbreezeCharacter>(OtherActor);
	if(Player)
	{
		Player->UpdateHealth(10.f);
	}
}